...
const foodPagar = () => {
  const pedidos = restaurant.consumption; // pegando os pedidos do cliente.
  const cardapioFood = restaurant.fetchMenu().food; // estou pegando os valores do cardapio food {coxinha: valor}.
  const cardapioDrink = restaurant.fetchMenu().drink; // estou pegando os valores do cardapio drink {agua: valor}.
  for (let pedido of pedidos) { // verificando o pedido do cliente com o cardapio e conferindo o valor dele no cardapio assim no final dar o valor total da conta. 
    if (cardapioFood[pedido]) {
      soma += cardapioFood[pedido];
    } else if (cardapioDrink[pedido]) {
      soma += cardapioDrink[pedido];
    }
  }
  return soma;
};
