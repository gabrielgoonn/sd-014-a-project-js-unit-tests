...
function pedido(str) { // Arrow function abusam da propriedade que as funções têm de poderem ser atribuídas a uma variável.
  this.consumption.push(str);
}
let conta = 0;
// this referencia o objeto que o contém.
// comecei do item4
const createMenu = (menu) => ({
  ...
  order: pedido, // item 8
  pay: function pay() {
    this.consumption.forEach((bill) => { // bill = consumption[i]
    if (menu.food[bill]) {
      conta += menu.food[bill];
    } 
  });
