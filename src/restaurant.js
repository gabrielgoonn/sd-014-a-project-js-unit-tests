...
const createMenu = (cardapio) => {
  const menu = {
    ...
    pay: () => {
      const pedido = menu.consumption;
      let total = 0;
      const produto = menu.fetchMenu();
      pedido.forEach((item) => {
        if (produto.food[item]) {
          total += produto.food[item];
        } else {
          total += produto.drink[item];
        }
      });
      const valor = Math.round(total * 1.1);
      return valor;
    },
  };
  return menu;
