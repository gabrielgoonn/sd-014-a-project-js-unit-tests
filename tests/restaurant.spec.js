...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    // ```
    const emptyinput = createMenu(); // Retorno: { fetchMenu: () => {}, ... }
    // ```
    assert.strictEqual(typeof emptyinput, 'object');
    // TESTE 2: Verifique que, dado que a função createMenu foi chamada com o objeto: `{ food: {}, drink: {} }`, 
    // verifique que 'objetoRetornado.fetchMenu()' retorna um objeto cujas chaves são somente `food` e `drink`.
    // ```
    const foodempty = createMenu({ food: {}, drink: {} });
    // objetoRetornado.fetchMenu() // Retorno: { food: {}, drink: {}}
    // ```
    assert.deepStrictEqual(foodempty.fetchMenu(), { food: {}, drink: {} });
    ...
    assert.deepStrictEqual(meuRestaurante.fetchMenu(), {food: {'coxinha': 3.90, 'sanduiche': 9.90},
    drinks: {'agua': 3.90, 'cerveja': 6.90},});
