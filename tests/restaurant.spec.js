...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    // ```
    const objetoRetornado3 = createMenu({ mtoloko: {}, adedonha: {} });
    // objetoRetornado.fetchMenu() // Retorno: objetoQualquer
    // ```
    assert.deepStrictEqual(objetoRetornado3.fetchMenu(), { mtoloko: {}, adedonha: {} });
    ...
    // ```
    const objetoRetornado8 = createMenu({ food: {coxinha: 10}, drink: {agua: 5} });
    objetoRetornado8.order('coxinha');
    objetoRetornado8.order('agua');
    objetoRetornado8.order('coxinha');
    // objetoRetornado.pay() // Retorno: somaDosPreçosDosPedidos
    // ```
    assert.strictEqual(objetoRetornado8.pay(), 25 * 1.1);
