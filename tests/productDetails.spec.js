...
describe('6 - Implemente os casos de teste para a função `productDetails`', () => {
  it('Verifica se a função `productDetails` tem o comportamento esperado', () => {
    ...
    // Source: https://www.w3schools.com/jsref/jsref_isarray.asp
    assert.strictEqual(Array.isArray(productDetails('camisa', 'calça')), true);

    // Teste que o array retornado pela função contém dois itens dentro.
    assert.strictEqual(Object.keys(productDetails('camisa', 'calça')).length, 2);

    // Teste que os dois itens dentro do array retornado pela função são objetos.
    const product = productDetails('camisa', 'calça');
