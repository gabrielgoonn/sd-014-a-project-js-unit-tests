...
describe('6 - Implemente os casos de teste para a função `productDetails`', () => {
  it('Verifica se a função `productDetails` tem o comportamento esperado', () => {
    const inputProduct = productDetails('Alcool gel', 'Máscara')
    // ESCREVA SEUS TESTES ABAIXO:
    // Teste que o retorno da função é um array.
    assert.strictEqual(inputProduct instanceof Array, true)
    // Teste que o array retornado pela função contém dois itens dentro.
    assert.equal(inputProduct.length, 2)
    // Teste que os dois itens dentro do array retornado pela função são objetos.
    assert.strictEqual(typeof (productDetails[0], [1]), 'object')
    // Teste que os dois objetos são diferentes entre si.
    assert.strictEqual((productDetails[0] !== [1]), true)
