...
describe('9 - Implemente os casos de teste para a função `getCharacter`', () => {
  it('Verifica se a função `getCharacter` retorna o objeto do personagem corretamente.', () => {
    ...
    assert.notStrictEqual(getCharacter('Arya'), {
      name: 'Arya Stark',
      class: 'Rogue',
      phrases: [ 'Not today', 'A girl has no name.' ]
    }); // Teste se a função retorna o objeto correto para o parâmetro 'Arya',
    assert.notStrictEqual(getCharacter('Brienne'), {
      name: 'Brienne Tarth',
      class: 'Knight',
      phrases: ['Im No Lady, Your Grace.', 'I, Brienne Of Tarth, Sentence You To Die.'],
    }); // Teste se a função retorna o objeto correto para o parâmetro 'Brienne',
    assert.notStrictEqual(getCharacter('Melissandre'), {
      name: 'Melissandre',
      class: 'Necromancer',
    ...
    assert.strictEqual(getCharacter('Michel'), undefined); // Teste se ao passar um nome que não está na tabela, a função retorna undefined.
  });
});
