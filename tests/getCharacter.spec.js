...
describe('9 - Implemente os casos de teste para a função `getCharacter`', () => {
  it('Verifica se a função `getCharacter` retorna o objeto do personagem corretamente.', () => {
    ...
    const resultsOfArya = () => {
      ...
      let obj = {};
      Object.keys(result).forEach((current) => obj[current] = result[current]);
      return obj;
    }
    assert.deepStrictEqual(resultsOfArya(), {
      name: 'Arya Stark',
      class: 'Rogue',
      phrases: [ 'Not today', 'A girl has no name.' ]
    });
    // Teste se a função retorna o objeto correto para o parâmetro 'Brienne',
    const resultsOfBrienne = () => {
      const result = getCharacter('Brienne');
      let obj = {};
      Object.keys(result).forEach((current) => obj[current] = result[current]);
      return obj;
    }
    assert.deepStrictEqual(resultsOfBrienne(), {
      name: 'Brienne Tarth',
      class: 'Knight',
      phrases: [ 'Im No Lady, Your Grace.', 'I, Brienne Of Tarth, Sentence You To Die.' ],
    });
    // Teste se a função retorna o objeto correto para o parâmetro 'Melissandre',
    const resultsOfMelissandre = () => {
      const result = getCharacter('Melissandre');
      let obj = {};
      Object.keys(result).forEach((current) => obj[current] = result[current]);
      return obj;
    }
    assert.deepStrictEqual(resultsOfMelissandre(), {
      name: 'Melissandre',
      class: 'Necromancer',
    ...
    assert.strictEqual(getCharacter('Jon'), undefined);
  });
});
