...
describe('9 - Implemente os casos de teste da função `getCharacter`', () => {
  it('Verifica se a função `getCharacter` retorna o objeto do personagem corretamente.', () => {
    ...
    assert.ok(getCharacter() === undefined, 'função sem parâmetro, retorna undefined.');
    // Teste se a função retorna o objeto correto para o parâmetro 'Arya',
    assert.deepStrictEqual(getCharacter('Arya'), { // 'O método deepStrictEqual testa se 02 objetos são iguais utilizando o operador ==='
        name: 'Arya Stark',
        class: 'Rogue',
        phrases: ['Not today', 'A girl has no name.']
    });
    ...
    assert.ok(getCharacter('Outro Nome') === undefined);
  });
});
